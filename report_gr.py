...
class PDF(FPDF):
    ...
    def add_graph(self, file_path):
        self.add_page()
        self.image(file_path, x=10, y=20, w=190)  # Adjust the position and size as needed

# Function to generate the graph
def generate_graph(ec02_values):
    x = np.arange(1, len(ec02_values) + 1)
    plt.figure(figsize=(10, 5))
    plt.plot(x, ec02_values, marker='o', linestyle='-', color='b')
    plt.xlabel('Index')
    plt.ylabel('ec02')
    plt.title('ec02 Values')
    plt.grid(True)
    plt.savefig('/Users/amogh1/PycharmProjects/pythonProject/pdfcreate/ec02_graph.png')
    plt.close()

# Collect user inputs and calculate ec02
def collect_data_and_calculate_ec02():
    wgoods = []
    d = []
    fmode = []
    for i in range(10):
        wgoods.append(float(input(f"Enter wgoods for data point {i+1}: ")))
        d.append(float(input(f"Enter d for data point {i+1}: ")))
        fmode.append(float(input(f"Enter fmode for data point {i+1}: ")))

    ec02_values = [w * di * f for w, di, f in zip(wgoods, d, fmode)]
    return ec02_values
...
# Collect data, calculate ec02, and generate graph
ec02_values = collect_data_and_calculate_ec02()
generate_graph(ec02_values)

# Add graph to PDF
pdf.add_graph('/Users/amogh1/PycharmProjects/pythonProject/pdfcreate/ec02_graph.png')
